...
public class EfficientDocument extends Document {
	...
	private void processText()
	{
		...
		// if the string in tokens is not a word for sure it is a sentence
		if (!tokens.isEmpty()) {
			for(String str : tokens) {
				//System.out.println(str);
				if (this.isWord(str)) {
					this.numSyllables += super.countSyllables(str);
					this.numWords += 1;
				} else {
					this.numSentences += 1;
				}
			...
			String lastWordInTheText = tokens.get(tokens.size() - 1);
			char lastChar = lastWordInTheText.charAt(lastWordInTheText.length() - 1); 
